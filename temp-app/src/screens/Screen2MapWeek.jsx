...
const ZONES = [
  { id: 'daily', label: 'I do this daily', icon: '●', color: '#d4a843', bg: 'rgba(212,168,67,0.06)' },
  { id: 'weekly', label: 'I do this weekly', icon: '◑', color: '#6aaa7a', bg: 'rgba(106,170,122,0.06)' },
  { id: 'rarely', label: 'I rarely do this', icon: '○', color: '#9896a4', bg: 'rgba(152,150,164,0.06)' },
]
...
function TaskPill({ task, isDragging = false, isOverlay = false }) {
  return (
    <div style={{
      padding: '10px 16px',
      background: isDragging ? '#1a1a2e' : '#fff',
      color: isDragging ? '#faf9f7' : '#2d2d3a',
      border: `1.5px solid ${isDragging ? '#1a1a2e' : '#e0ddd5'}`,
      borderRadius: 100,
      fontSize: 14,
      fontWeight: 500,
      ...
      cursor: 'grab',
      userSelect: 'none',
      boxShadow: isDragging || isOverlay ? '0 8px 24px rgba(26,26,46,0.18)' : '0 1px 3px rgba(0,0,0,0.06)',
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: 220,
        borderRadius: 16,
        border: `2px dashed ${isOver ? zone.color : '#ddd9d1'}`,
        background: isOver ? zone.bg : 'rgba(250,249,247,0.5)',
        padding: 20,
...
export default function Screen2MapWeek({ roleKey, onNext, onBack }) {
  ...
  function handleDragEnd(e) {
    ...
    if (zoneId) {
      setFrequencies(prev => ({ ...prev, [active.id]: zoneId }))
    }
  }
  ...
  function quickFill() {
    const remaining = {}
    tasks.forEach(t => {
      if (!frequencies[t.id]) remaining[t.id] = t.defaultFrequency
    })
    // Stagger the fills for animation
    const entries = Object.entries(remaining)
    entries.forEach(([id, freq], i) => {
      setTimeout(() => {
  ...
  function demoMode() {
    ...
    setTimeout(() => {
      tasks.forEach((t, i) => {
        setTimeout(() => {
          setFrequencies(prev => ({ ...prev, [t.id]: t.defaultFrequency }))
        }, i * 160)
      })
    }, 100)
  }
  ...
  function handleProceed() {
    // Merge sorted frequencies with defaults for unsorted tasks
    const allFreqs = {}
    tasks.forEach(t => {
      allFreqs[t.id] = frequencies[t.id] || t.defaultFrequency
    })
    onNext(allFreqs)
  }

  return (
    <div style={{ minHeight: '100vh', background: '#faf9f7', padding: '24px 32px 40px' }}>
      {/* Top bar */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, maxWidth: 1200, margin: '0 auto 32px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 13, color: '#9896a4', fontFamily: 'Outfit, sans-serif',
            }}
          >
            Demo mode
          </button>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        >
          <h2 className="font-display" style={{
            fontSize: 'clamp(26px, 3vw, 38px)', fontWeight: 400, color: '#1a1a2e',
            marginBottom: 12, lineHeight: 1.2,
          }}>
            Here are the common tasks for your role.
            <br />
            <span style={{ color: '#d4a843' }}>Drag them to match your actual week.</span>
          </h2>
          <p style={{ color: '#9896a4', fontSize: 15, fontWeight: 400 }}>
            This takes 60 seconds and makes your results personal.
          </p>
        </motion.div>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            <div style={{
              width: 220, height: 6, background: '#e8e5e0', borderRadius: 3, overflow: 'hidden',
            }}>
              <motion.div
                style={{ height: '100%', background: '#6aaa7a', borderRadius: 3 }}
                animate={{ width: `${(sortedCount / tasks.length) * 100}%` }}
                transition={{ duration: 0.3, ease: 'easeOut' }}
              />
            </div>
            <span style={{ fontSize: 13, color: '#9896a4', fontWeight: 500 }}>
        ...
        >
          ...
          {unsortedTasks.length > 0 && (
            ...
            >
              <p style={{ fontSize: 12, fontWeight: 700, color: '#9896a4', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>
          ...
          <DragOverlay>
            {activeTask ? (
              <TaskPill task={activeTask} isDragging isOverlay />
            ) : null}
          </DragOverlay>
        </DndContext>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
          ...
          <motion.button
            ...
            disabled={!canProceed}
            whileHover={canProceed ? { scale: 1.02, backgroundColor: '#252545' } : {}}
            ...
            style={{
              ...
              fontFamily: 'Outfit, sans-serif',
              fontWeight: 600,
              background: canProceed ? '#1a1a2e' : '#d6d3cc',
              color: canProceed ? '#faf9f7' : '#9896a4',
              border: 'none',
              borderRadius: 12,
